...
# #         rto_file     = request.files.get('rto_file')
# #         rules_json   = request.form.get('rules_json', '').strip()

# #         # Optional: filter rows by selected states via rto_group_id → rto_code prefix
# #         selected_states_raw = request.form.get('selected_states', '').strip()
# #         selected_states = [s.strip().upper() for s in selected_states_raw.split(',') if s.strip()] if selected_states_raw else []
...
# #         if not rto_file or rto_file.filename == '':
# #             return jsonify({'success': False, 'error': 'RTO Mapping file is required'}), 400
# #         if not rules_json:
...
# #             # Strip LOCATION/ODISHA rules — user said ignore location for now
# #             rules = [r for r in rules if not r.get('LOCATION')]
...
# #         with tempfile.NamedTemporaryFile(suffix='.xlsx', delete=False) as tmp_rto:
# #             rto_file.save(tmp_rto.name)
# #             rto_path = tmp_rto.name

# #         # ── 4. Build Odisha lookup ────────────────────────────────────────────
# #         odisha_ids = calc.build_rto_odisha_lookup(rto_path)
...
# #         # ── 5b. Apply state filter via RTO mapping ────────────────────────────
# #         if selected_states:
# #             # Build a set of rto_group_ids that belong to any selected state
# #             try:
# #                 rto_df = pd.read_excel(rto_path)
# #                 rto_df.columns = [c.strip() for c in rto_df.columns]
# #                 # Filter rto codes whose 2-letter prefix matches a selected state
# #                 def _state_prefix(code):
# #                     import re as _re
# #                     m = _re.match(r'^([A-Z]{2})', str(code).strip().upper())
# #                     return m.group(1) if m else ''
# #                 rto_df['_prefix'] = rto_df['rto_code'].apply(_state_prefix)
# #                 allowed_group_ids = set(
# #                     rto_df[rto_df['_prefix'].isin(selected_states)]['rto_group_id']
# #                     .dropna().astype(int).tolist()
# #                 )
# #                 if allowed_group_ids:
# #                     df['_rto_int'] = pd.to_numeric(df['rto_group_id'], errors='coerce').fillna(-1).astype(int)
# #                     df = df[df['_rto_int'].isin(allowed_group_ids)].drop(columns=['_rto_int'])
# #                     df = df.reset_index(drop=True)
# #             except Exception as filter_err:
# #                 print(f"[WARN] State filter failed, processing all rows: {filter_err}")

# #         total = len(df)
# #         print(f"[/process] Rows after state filter: {total} (was {total_before_filter})")
...
# #         # ── 6. Monkey-patch IRDA thresholds if user customised them ──────────
...
# #                             rules_arg, is_od=True, odisha_ids=None):
# #             explanation = {
# #                 "lob": "", "segment": "", "insurer_matched": "",
# #                 "po_formula": "", "remarks_slab": "", "odisha_deduction": "",
# #                 "calculation_note": "",
# #             }
# #             try:
# #                 p = float(payin)
...
# #                                 rto_group_name, vehicle_type_id, from_wt, to_wt,
# #                                 rules_arg, is_od=is_od, odisha_ids=odisha_ids)
...
# #                     payin_od, sp, seg, ins, rto_grp_id, vt, f_wt, t_wt,
# #                     rules, is_od=True, odisha_ids=odisha_ids
# #                 )
...
# #             od_slab.append(expl_od['remarks_slab']); od_odisha.append(expl_od['odisha_deduction'])
...
# #                     payin_tp, sp, seg, ins, rto_grp_id, vt, f_wt, t_wt,
# #                     rules, is_od=False, odisha_ids=odisha_ids
# #                 )
...
# #             tp_slab.append(expl_tp['remarks_slab']); tp_odisha.append(expl_tp['odisha_deduction'])
...
# #         location_rules = [r for r in rules if r.get('LOCATION')]
# #         normal_rules   = [r for r in rules if not r.get('LOCATION')]
# #         lobs = sorted(set(r.get('LOB', '') for r in normal_rules if r.get('LOB')))
# #         return jsonify({
# #             'valid': True,
# #             'total_rules': len(rules),
# #             'normal_rules': len(normal_rules),
...
#         try:
#             rules = json.loads(rules_json)
#             # Strip any hardcoded LOCATION/ODISHA rules — we use only frontend-built ones
#             rules = [r for r in rules if not r.get('LOCATION')]
#         except json.JSONDecodeError as e:
#             return jsonify({'success': False, 'error': f'Invalid JSON: {e}'}), 400

#         # ── 2b. Merge in location rules from the rule builder ─────────────────
#         location_rules_raw = request.form.get('location_rules_json', '').strip()
#         if location_rules_raw:
#             try:
#                 loc_rules = json.loads(location_rules_raw)
#                 rules = rules + loc_rules   # append location rules at end
#                 print(f"[/process] Merged {len(loc_rules)} location rules from UI")
#             except json.JSONDecodeError as e:
#                 print(f"[WARN] Could not parse location_rules_json: {e}")
...
#         # ── 6. Build state prefix lookup from RTO mapping ─────────────────────
#         # Map rto_group_id → 2-letter state prefix (e.g. 101 → "GJ")
#         rto_group_to_state = {}
#         try:
#             rto_df2 = pd.read_excel(rto_path)
#             rto_df2.columns = [c.strip() for c in rto_df2.columns]
#             import re as _re
#             for _, rrow in rto_df2.iterrows():
#                 code = str(rrow.get('rto_code', '')).strip().upper()
#                 gid  = rrow.get('rto_group_id')
#                 m    = _re.match(r'^([A-Z]{2})', code)
#                 if m and gid is not None:
#                     try:
#                         rto_group_to_state[int(gid)] = m.group(1)
#                     except (ValueError, TypeError):
#                         pass
#         except Exception as e:
#             print(f"[WARN] State prefix lookup failed: {e}")
...
#         # Also build a location-rule lookup: state_prefix → list of rules
#         import re as _re2
#         loc_rules_by_state = {}
#         for r in rules:
#             loc = r.get('LOCATION', '').strip().upper()
#             if loc:
#                 if loc not in loc_rules_by_state:
#                     loc_rules_by_state[loc] = []
#                 loc_rules_by_state[loc].append(r)

#         def apply_location_override(p_out, p, rto_grp_id):
#             """If the row's state has location rules, apply them and return new p_out."""
#             state_prefix = rto_group_to_state.get(int(rto_grp_id) if rto_grp_id is not None else -1)
#             if not state_prefix:
#                 return p_out
#             loc_rules = loc_rules_by_state.get(state_prefix, [])
#             if not loc_rules:
#                 return p_out
#             # Filter by insurer (All Companies always matches)
#             candidate = [r for r in loc_rules
#                          if str(r.get('INSURER','')).strip().upper() == 'ALL COMPANIES']
#             ded_po = calc.select_po(candidate, p)
#             if not ded_po:
#                 return p_out
#             m = _re2.search(r'-(\d+(?:\.\d+)?)%', str(ded_po).upper())
#             if m:
#                 ded = float(m.group(1))
#                 return calc.floor_payout(p - ded)
#             return p_out
...
#                     rules, is_od=True, odisha_ids=odisha_ids
#                 )
#                 if loc_rules_by_state:
#                     calc_od = apply_location_override(calc_od, float(payin_od), rto_grp_id)
#                 new_od.append(calc_od)
#                 if abs(float(old_od) - calc_od) > 0.001:
#                     changed_od += 1
#             else:
...
#                 )
#                 if loc_rules_by_state:
#                     calc_tp = apply_location_override(calc_tp, float(payin_tp), rto_grp_id)
#                 new_tp.append(calc_tp)
#                 if abs(float(old_tp) - calc_tp) > 0.001:
#                     changed_tp += 1
#             else:
